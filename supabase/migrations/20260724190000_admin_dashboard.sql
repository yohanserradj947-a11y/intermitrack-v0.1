-- Tableau de bord admin PRIVÉ (Yohan uniquement). SECURITY DEFINER + garde sur l'email admin.
-- Calcule : inscrits (total + nouveaux), DAU/WAU/MAU, rétention 7 j, heures passées dans l'appli
-- (estimées par sessions, gap > 30 min = nouvelle session), le tout EN EXCLUANT le compte de Yohan.
...
begin
  ...
  marked as (
    select user_id, created_at,
           case when prev is null or created_at - prev > interval '30 minutes' then 1 else 0 end as new_sess
    from ordered
  ),
  sess as (
    select user_id, created_at,
           sum(new_sess) over (partition by user_id order by created_at) as sess_id
    from marked
  ),
  sess_dur as (
    select user_id, sess_id, max(created_at) - min(created_at) as dur
    from sess group by user_id, sess_id
  )
  select json_build_object(
    ...
    'retention_7d', (
      select case when denom = 0 then 0 else round(100.0 * num / denom) end from (
        select
          count(*) filter (where u.created_at <= now() - interval '7 days') as denom,
          count(*) filter (where u.created_at <= now() - interval '7 days'
                             and exists (select 1 from ev where ev.user_id = u.id and ev.created_at >= now() - interval '7 days')) as num
    ...
    'hours_per_user',     (select case when count(distinct user_id) = 0 then 0
                             else round((extract(epoch from sum(dur))/3600.0 / count(distinct user_id))::numeric, 1) end from sess_dur),
    ...
    'signups_by_day', (
      select coalesce(json_agg(d order by d->>'jour'), '[]'::json) from (
        select json_build_object('jour', to_char(date_trunc('day', created_at), 'YYYY-MM-DD'), 'n', count(*)) as d
        from auth.users where id <> me and created_at >= now() - interval '30 days'
        group by date_trunc('day', created_at)
      ) x
    ),
    'top_views', (
      select coalesce(json_agg(t), '[]'::json) from (
        select replace(event_name, 'view_', '') as vue, count(*) as n
        from ev where event_name like 'view_%'
