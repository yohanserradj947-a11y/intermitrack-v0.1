...
//  • Rappel d'actualisation France Travail : le 28 puis l'avant-dernier jour du mois (J-1 fin), sans
//    chercher à savoir si c'est déjà fait.
...
function buildItems(missions: { mission_date: string; production?: string }[]): Item[] {
  ...
  // 2) Rappels d'actualisation : le 28 puis l'avant-dernier jour du mois, 6 mois à l'avance.
  for (let i = 0; i < 6; i++) {
    const y = now.getFullYear(), mo = now.getMonth() + i;
    const d28 = new Date(y, mo, 28, 11, 0, 0);
    if (d28 > now) items.push({ date: d28, title: 'Actualisation France Travail', body: "C'est l'heure de t'actualiser sur ton espace France Travail 📋" });
    const lastDay = new Date(y, mo + 1, 0).getDate();
    const jm1 = new Date(y, mo, lastDay - 1, 11, 0, 0);
    if (jm1 > now && jm1.getDate() !== 28) items.push({ date: jm1, title: 'Dernier rappel actualisation', body: "Pense à t'actualiser avant la fin du mois ⏰" });
  }

  items.sort((a, b) => a.date.getTime() - b.date.getTime());
  return items.slice(0, MAX_PENDING);
}
