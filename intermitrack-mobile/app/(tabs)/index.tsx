...
import { StyleSheet, View, Text, ScrollView, TextInput, TouchableOpacity, ActivityIndicator, StatusBar, Platform } from 'react-native';
...
function ProgressChart({done,planned,total}:{done:number;planned:number;total:number}){
  const doneP=Math.min(done/total,1);
  const planP=Math.min(planned/total,1-doneP);
  const restP=Math.max(0,1-doneP-planP);
  const totalPct=Math.round((doneP+planP)*100);
  const donePct=Math.round(doneP*100);
  const planPct=Math.round(planP*100);
  return(
    <View style={pc.wrap}>
      <View style={pc.centerBox}>
        <Text style={pc.pct}>{totalPct}%</Text>
        <Text style={pc.sub}>potentiel total</Text>
      </View>
      <View style={pc.barWrap}>
        <View style={[pc.seg,{flex:doneP||0.001,backgroundColor:C.petrol,borderTopLeftRadius:99,borderBottomLeftRadius:99}]}/>
        {planP>0&&<View style={[pc.seg,{flex:planP,backgroundColor:C.orange}]}/>}
        {restP>0&&<View style={[pc.seg,{flex:restP,backgroundColor:'#E2E8F0',borderTopRightRadius:99,borderBottomRightRadius:99}]}/>}
      </View>
      <View style={pc.legends}>
        <View style={pc.leg}><View style={[pc.dot,{backgroundColor:C.petrol}]}/><Text style={pc.legTxt}>Effectué · {donePct}%</Text></View>
        <View style={pc.leg}><View style={[pc.dot,{backgroundColor:C.orange}]}/><Text style={pc.legTxt}>Prévu · {planPct}%</Text></View>
        <View style={pc.leg}><View style={[pc.dot,{backgroundColor:'#D8E4DF'}]}/><Text style={[pc.legTxt,{color:C.muted}]}>Restant</Text></View>
      </View>
    </View>
  );
}
const pc=StyleSheet.create({
  wrap:{padding:12},
  centerBox:{alignItems:'center',marginBottom:16},
  pct:{fontSize:52,fontWeight:'900',color:C.petrol,letterSpacing:-2},
  sub:{fontSize:13,color:C.muted,marginTop:-4},
  barWrap:{flexDirection:'row',height:22,borderRadius:99,overflow:'hidden',backgroundColor:'#E2E8F0',marginBottom:12},
  seg:{},
  legends:{flexDirection:'row',justifyContent:'space-between',flexWrap:'wrap',gap:6},
...
export default function HomeScreen(){
  ...
  useEffect(()=>{
    ...
    supabase.auth.onAuthStateChange((_e:any,s:any)=>setSession(s));
  },[]);
  ...
  const totalGross=missions.reduce((a:number,m:any)=>a+Number(m.gross_amount||0),0);
  const totalPages=Math.ceil(upcoming.length/6);
  const visibleM=upcoming.slice(missionPage*6,(missionPage+1)*6);
  ...
  if(!session)return(
    <ScrollView contentContainerStyle={s.authPage} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.authBrand}>
        ...
        <Text style={s.authMainline}>{"Toute votre\nintermittence\nau même endroit."}</Text>
        <Text style={s.authIntro}>{"Suivi des missions, heures ARE,\ndocuments et prévisions."}</Text>
      </View>
      <View style={s.authCard}>
        <View style={s.authTabs}>
          <View style={[s.authTab,s.authTabActive]}><Text style={s.authTabTxtActive}>Connexion</Text></View>
          <View style={s.authTab}><Text style={s.authTabTxt}>Créer un compte</Text></View>
        </View>
        <Text style={s.formLabel}>Email</Text>
        <TextInput style={s.input} placeholder="votre@email.com" placeholderTextColor={C.muted} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"/>
        <Text style={s.formLabel}>Mot de passe</Text>
        <TextInput style={s.input} placeholder="Minimum 6 caractères" placeholderTextColor={C.muted} value={password} onChangeText={setPassword} secureTextEntry/>
        <TouchableOpacity style={s.primaryBtn} onPress={signIn} disabled={authLoading}>
          <Text style={s.primaryBtnTxt}>{authLoading?'Connexion…':'Se connecter'}</Text>
        </TouchableOpacity>
      </View>
      <Text style={s.authSecure}>🔒 Données chiffrées — accès uniquement à votre compte</Text>
    </ScrollView>
  );
  ...
  return(
    <ScrollView style={s.container} contentContainerStyle={{paddingBottom:40}}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.avatarBtn} onPress={signOut}>
          ...
          <DateTimePicker
            ...
            onChange={async(_e:any,date?:Date)=>{
              ...
              if(date){
                ...
                await AsyncStorage.setItem('intermitrack_are_date',iso);
                setAreDate(iso);
              }
            }}
          />
      ...
      <View style={s.chartCard}>
        <ProgressChart done={doneH} planned={planH} total={507}/>
      </View>
...
const s=StyleSheet.create({
  ...
  authPage:{flexGrow:1,backgroundColor:C.petrol,paddingHorizontal:22,paddingTop:64,paddingBottom:32},
  authBrand:{alignItems:'center',marginBottom:28},
  authMainline:{fontSize:24,fontWeight:'900',color:'white',textAlign:'center',lineHeight:30,letterSpacing:-0.5,marginTop:14},
  authIntro:{fontSize:13,color:'rgba(255,255,255,.65)',textAlign:'center',marginTop:8,lineHeight:18},
  authCard:{backgroundColor:C.card,borderRadius:22,padding:22},
  authTabs:{flexDirection:'row',gap:8,marginBottom:14},
  authTab:{flex:1,paddingVertical:11,borderRadius:14,backgroundColor:C.soft,alignItems:'center'},
  authTabActive:{backgroundColor:C.petrol},
  authTabTxt:{fontSize:13,fontWeight:'800',color:C.petrol},
  authTabTxtActive:{fontSize:13,fontWeight:'800',color:'white'},
  formLabel:{fontWeight:'700',fontSize:13,color:C.text,marginTop:12,marginBottom:6},
  ...
  primaryBtn:{backgroundColor:C.petrol,borderRadius:15,paddingVertical:14,alignItems:'center',marginTop:14},
  primaryBtnTxt:{color:'white',fontWeight:'800',fontSize:15},
  authSecure:{color:'rgba(255,255,255,.5)',fontSize:12,textAlign:'center',marginTop:16},
